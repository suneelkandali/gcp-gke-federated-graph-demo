...
function upgradeSubgraphsIfNecessary(inputs) {
    ...
    let errors = [];
    const fed2InterfaceObjectTypesToSubgraphs = new utils_1.SetMultiMap();
    const fed1InterfaceKeyTypesToSubgraphs = new utils_1.SetMultiMap();
    ...
    for (const subgraph of inputs.values()) {
        if (subgraph.isFed2Subgraph()) {
            ...
            for (const application of subgraph.metadata().interfaceObjectDirective().applications()) {
                const typeName = application.parent.name;
    ...
    for (const [typeName, interfaceObjectSubgraphs] of fed2InterfaceObjectTypesToSubgraphs) {
        ...
        if (interfaceKeySubgraphs) {
            errors.push(error_1.ERRORS.INTERFACE_OBJECT_USAGE_ERROR.err(`The @interfaceObject directive is used on type "${typeName}" in ${(0, federation_1.printSubgraphNames)([...interfaceObjectSubgraphs])}, which requires other subgraphs to resolve its type name via an interface @key. However, @key on an interface in a federation 1 subgraph does not mean it can fulfill the __typename-resolution requirement that @interfaceObject depends on. For ${(0, federation_1.printSubgraphNames)([...interfaceKeySubgraphs])}, either upgrade them to federation 2 subgraphs or remove @key from the type.`));
        }
    }
...
class SchemaUpgrader {
    constructor(originalSubgraph, allSubgraphs, objectTypeMap) {
        ...
        this.errors = [];
        this.interfaceKeyTypes = new Set();
