            ...
            switch (key) {
                case 'name':
                    ...
                    if (!importRegExp.test(value)) {
                        errors.push(error_1.ERRORS.INVALID_LINK_DIRECTIVE_USAGE.err(`Invalid value for the "name" field for sub-value ${(0, values_1.valueToString)(elt)} of @link(import:) argument: must use a GraphQL name.`, { nodes: directive.sourceAST }));
                        continue importArgLoop;
                    }
                    name = value;
                    break;
                case 'as':
                    ...
                    if (!importRegExp.test(value)) {
                        errors.push(error_1.ERRORS.INVALID_LINK_DIRECTIVE_USAGE.err(`Invalid value for the "as" field for sub-value ${(0, values_1.valueToString)(elt)} of @link(import:) argument: must use a GraphQL name.`, { nodes: directive.sourceAST }));
                        continue importArgLoop;
                    }
                    break;
                default:
...
class FeatureDefinitions {
    ...
    latestNonPreview() {
        var _a;
        (0, utils_1.assert)(this._definitions.length > 0, 'Trying to get latest when no definitions exist');
        return (_a = this._definitions.find(def => !this._previewVersions.has(def.version.toString()))) !== null && _a !== void 0 ? _a : this._definitions[0];
    }
